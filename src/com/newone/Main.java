...
public class Main {
	...
	//BM25得分计算
	public static double BM25(double tf,int n_qi,int dl,double avgdl){
		int N=docLen.size();
		...
		for (String[] t : myquery) {
			...
			for (int i = 0; i < t.length; i++) {
				if (Index.containsKey(t[i])) {
					...
					for (Document dd : Index.get(t[i])) {
						//double tf =dd.freq;//词频指次出现的次数还是频率
						double tf=(double)dd.freq/(double)dd.length;
						int dl = dd.length;
						int n_qi = Index.get(t[i]).size();
						double avgdl=len/n_qi;
						double score = BM25(tf, n_qi, dl,avgdl);
...
//		avgLength();
		//System.out.println("词表中词的数量："+KeyWords.size());
//		for (Entry<String, List<Document>> entry : Index.entrySet()) {
